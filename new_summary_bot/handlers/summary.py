...
async def cmd_summary(message: Message, state: FSMContext):
    ...
    if not posts:
        await message.answer("Вряд ли есть что-то новое… попробуй позже.")
        return
    ...
    if "Не удалось сгенерировать" in summary_text:
        # Предлагаем «Повторить» или «Укоротить»
...
async def retry_summary_cb(call: CallbackQuery, state: FSMContext):
    ...
    # Для очередного вызова cmd_summary, либо отдельной функции
...
async def short_summary_cb(call: CallbackQuery, state: FSMContext):
    """
    Сделать «укороченную версию» – напр. берём только первые 5 сообщений.
    """
    await state.clear()
